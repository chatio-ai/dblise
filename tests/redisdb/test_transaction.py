...
async def _ledger(facade: RedisFacade) -> AsyncGenerator[Ledger]:
...
async def test_empty_assign(facade: RedisFacade, ledger: Ledger, *, transaction: bool) -> None:
...
async def test_atomic_immediate(facade: RedisFacade, ledger: Ledger) -> None:
...
async def test_atomic_connections(facade: RedisFacade, ledger: Ledger) -> None:
    pool = facade.broker.client.connection_pool

    async def assign(right: Record[Counter]) -> None:
        await right.assign(Counter(2, note='now'))

    await facade.atomic(assign, ledger.right, watches=(ledger.left,))
    assert sum(count for count, _ in pool.get_connection_count()) == 1


async def test_atomic_transfer(facade: RedisFacade, ledger: Ledger) -> None:
    await ledger.left.assign(Counter(1))
...
async def test_atomic_retries(
        facade: RedisFacade, other_facade: RedisFacade, ledger: Ledger) -> None:
    reads: list[int] = []
...
async def test_atomic_atomic(
        facade: RedisFacade, ledger: Ledger, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with monkeypatch.context() as patch:
        patch.setattr(client.Pipeline, 'hdel', _raise)
...
async def test_atomic_connect(
    ...
    client_id = await facade.broker.client.client_id()

    async def drop(right: Record[Counter]) -> None:
        value = await right.value()
        drops.append(value.count)
        if len(drops) == 1:
            await other_facade.broker.client.client_kill_filter(_id=client_id)
            await right.value()
...
async def test_pipeline_refuse(facade: RedisFacade, ledger: Ledger, *, transaction: bool) -> None:
...
async def test_pipeline_nested(facade: RedisFacade, ledger: Ledger, *, transaction: bool) -> None:
    async with facade.pipeline(ledger.empty, transaction=transaction) as (empty,):
        with pytest.raises(TypeError):
...
async def test_modify_conflict(ledger: Ledger, other_facade: RedisFacade) -> None:

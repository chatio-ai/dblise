...
class RedisScores(RedisEntity, Scores):
    ...
    def values(self, *, reverse: bool = False) -> Awaitable[Sequence[str]]:
        return RedisResult.same(
            self._broker, lambda redis: redis.zrange(self._key_path, 0, -1, desc=reverse))
    ...
    def score(self, key: str) -> Awaitable[float | None]:
        return RedisResult.same(
            self._broker, lambda redis: redis.zscore(self._key_path, key))

    @override
    def count(self) -> Awaitable[int]:
        return RedisResult.same(
            self._broker, lambda redis: redis.zcount(self._key_path, -math.inf, math.inf))

    @override
    def len(self) -> Awaitable[int]:
        return RedisResult.same(
            self._broker, lambda redis: redis.zcard(self._key_path))
    ...
    def remove(self, key: str) -> Awaitable[bool]:
        return RedisResult(
            self._broker, lambda redis: redis.zrem(self._key_path, key), bool)
